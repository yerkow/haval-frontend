...
function Dashboard() {
  ...
  const handleData = (type) => {
    if (type == "Users") {
      axios.get("http://78.40.109.168:8000/api/users").then((response) => {
        setHeader(["id", "firstname", "lastname", "telephone", "email"]);
        setData(response.data.data);
        setType("users");
      });
    } else if (type == "Cars") {
      axios.get("http://78.40.109.168:8000/api/cars").then((response) => {
        setHeader([
          "id",
          "brand",
          "model",
      ...
      axios.get("http://78.40.109.168:8000/api/rents").then((response) => {
        setHeader([
          "id",
          "rental_date",
          "return_date",
  ...
  const handleUpdateItem = (itemId, updatedItem) => {
    const endpoint = `http://78.40.109.168:8000/api/${type}/${itemId}`;
  ...
  const handleDelete = (id) => {
    const endpoint = `http://78.40.109.168:8000/api/${type}/${id}`;
